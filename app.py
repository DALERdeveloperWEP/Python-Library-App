...
def show_books(library: list[list[str, str, int, bool]]) -> None:
    ...
    for index, lib in enumerate(library, start=1):
        if lib[3]:
            ...
            table.add_row(str(index), lib[0],
                          lib[1], str(lib[2]), "O'qilmagan")
...
def delete_book(library: list[list[str, str, int, bool]]) -> None:
    ...
    if library:
        ...
        if get_delete_book_num > 0 and get_delete_book_num <= len(library):
            ...
            print("Siz notogri kitob raqamini tanladingiz!")
    else:
        print("Sizda kitob Mavjud emas!")
...
def change_status(library: list[list[str, str, int, bool]]) -> None:
    ...
    if library:
        ...
        if get_status_book_num > 0 and get_status_book_num <= len(library):
            library[get_status_book_num -
                    1][3] = not library[get_status_book_num - 1][3]
            show_books(library)
        else:
            print("Siz notogri kitob raqamini tanladingiz!", len(library))
    else:
        print("Sizda kitob Mavjud emas!")
        ...
        if search in lib[0].lower() or search in lib[1].lower() or search in str(lib[2]):
            ...
            if lib[3]:
                search_table.add_row(
                    str(index), lib[0], lib[1], str(lib[2]), "O'qilgan")
            else:
                search_table.add_row(
                    str(index), lib[0], lib[1], str(lib[2]), "O'qilmagan")
...
def filter_books(library: list[list[str, str, int, bool]]) -> None:
    ...
    Args:
        library: Kitoblar ro'yxati - har bir kitob [title, author, year, status] formatida
    """
    oqigan_kitob_table = Table(
        title="O'qilgan Kitoblar", title_style="bold green on green")
    ...
    oqimagan_kitob_table = Table(
        title="O'qilmagan Kitoblar", title_style="bold green on yellow")
    ...
    for index, lib in enumerate(library, start=1):
        if lib[3]:
            oqigan_kitob_table.add_row(
                str(index), lib[0], lib[1], str(lib[2]), "O'qilgan")
        else:
            oqimagan_kitob_table.add_row(
                str(index), lib[0], lib[1], str(lib[2]), "O'qilmagan")
...
def main():
    ...
    while True:
        ...
        if choice == '1':
            ...
            manu(), console.print("bunday menyu mavjud emas!", style="red")
